...
(*
SearchAbout rev.
rev_snoc: forall (X : Type) (v : X) (s : list X), rev (snoc s v) = v :: rev s
...
Proof.
  intros. induction l'.
  Case "[]". rewrite H. reflexivity.
  Case "cons". rewrite H. symmetry. apply rev_involutive.
Qed.

(* Here is an example where picking the wrong induction variable 
   can make things a little harder. It's not much, but it's still harder. *)
Theorem rev_exercise1' : forall (l l' : list nat),
     l = rev l' ->
     l' = rev l.
Proof.
  intros. induction l.
  Case "[]". rewrite H. symmetry. apply rev_involutive. 
  Case "cons". rewrite H. symmetry. apply rev_involutive.
Qed.
...
Proof.
  intros.
  inversion H. inversion H0. apply trans_eq with x. symmetry. apply H2. reflexivity.
Qed.
...
Theorem plus_plus_zero: forall m, m + m = 0 -> m = 0.
Proof.
